...
def cfg_to_rsm(cfg):
    ...
    for production in cfg.productions:
        regex = (
            ...
            else Epsilon().to_text()
        )
        productions[Symbol(production.head)].append(regex)
...
def ebnf_to_rsm(ebnf):
    strip = lambda x: x.strip()
    productions = defaultdict(list)
    for line in map(strip, ebnf.splitlines()):
        parts = [*map(strip, line.split("->"))]
        if len(parts) == 2:
            head, body = parts
            body = body if body != "" else Epsilon().to_text()
...
def cfpq_with_tensor(
    ...
    while True:
        ...
        test = lambda x: x in rsm_matrix.start_states and x in rsm_matrix.final_states
        for i, j in closure:
            if test(rsm_matrix.get_index(i)):
...
def rsm_to_matrix(rsm: RecursiveAutomaton) -> tuple:
    states, epsilon_symbols = set(), set()
    automaton = FiniteAutomaton()

    for v, p in rsm.boxes.items():
        for s in p.dfa.start_states:
            automaton.start_states.add(State((v, s.value)))
        for s in p.dfa.final_states:
